...
-- 2017-01-01,AAvenlaBudnuchit,Created File
--**************************************************************************--
Begin Try
	Use Master;
	If Exists(Select Name From SysDatabases Where Name = 'Assignment06DB_AAvenlaBudnuchit')
	 Begin 
	  Alter Database [Assignment06DB_AAvenlaBudnuchit] set Single_user With Rollback Immediate;
	  Drop Database Assignment06DB_AAvenlaBudnuchit;
	 End
	Create Database Assignment06DB_AAvenlaBudnuchit;
End Try
Begin Catch
	Print Error_Number();
End Catch
go
Use Assignment06DB_AAvenlaBudnuchit;
...
AS
	SELECT DISTINCT TOP 100
		EmployeeName = dbo.Employees.EmployeeFirstName + ' ' + 	dbo.Employees.EmployeeLastName,
		dbo.Inventories.InventoryDate
	FROM dbo.Inventories
	JOIN dbo.Employees
...
AS
	...
	JOIN dbo.Inventories
	ON dbo.Inventories.ProductID = dbo.Products.ProductID 
	JOIN dbo.Employees
	ON dbo.Employees.EmployeeID = dbo.Inventories.EmployeeID
	WHERE dbo.Products.ProductName IN ('Chai', 'Chang')
	ORDER BY 3, 1, 2
Go
...
AS
	SELECT TOP 100
		...
		dbo.Inventories.InventoryID,
		dbo.Inventories.InventoryDate,
		dbo.Inventories.[Count],
